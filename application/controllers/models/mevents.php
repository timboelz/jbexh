...
Class mevents extends CI_Model {
	...
	function list_vacancy($where) {
			...
			if ($where != '') {
                    $sql .= " WHERE a.id='".$where."' ORDER BY a.id ASC ";
                }
	...
	function listevents($limit=0,$offset=0) {
		$sql = $this->db->query("SELECT 
								  ...
								  MONTHNAME(te.event_date) as month
							 FROM tbl_event te 
								  INNER JOIN tbl_event_detail ted ON te.event_id=ted.event_id
								  WHERE YEAR(curdate()) = YEAR(te.event_date)
								  and MONTH(te.event_date) >= MONTH(curdate()) 
